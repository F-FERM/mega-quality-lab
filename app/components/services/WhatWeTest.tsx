...
const defaultData: ServicesHeroData = {
  sectionTitle: "What We Test",
  heroTitle: "OUR TESTING & INVESTIGATION",
  heroSubtitle: "SERVICES",
  heroImage: "",
  heroImageAlt: "Inspecting documents with a magnifying glass",
  heroInlineLinks: [],
  description:
...
function WhatWeTestBannerSkeleton() {
  return (
    <section
...
function WhatWeTestBanner() {
  const [data, setData] = useState<ServicesHeroData>(defaultData);
  ...
  useEffect(() => {
    const fetchHero = async () => {
      try {
        ...
        console.error("Failed to fetch Services hero banner:", err);
        setData(defaultData);
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return <WhatWeTestBannerSkeleton />;
  }
